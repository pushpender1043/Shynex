...
function Nav() {
  ...
  return (
    <div className='w-full h-[70px] fixed top-0 left-0 z-50 bg-[#EBD9D1] shadow-md flex items-center justify-between px-6'>
      {/* Logo */}
      <div className='flex items-center gap-3'>
        <img src={logo} alt="logo" className='w-[30px]' />
        <h1 className='text-xl font-bold text-black'>OneCart</h1>
      </div>

      {/* Menu */}
      <div className='hidden md:flex gap-4 text-white'>
        <li className='text-xs bg-black/80 px-4 py-2 rounded-xl cursor-pointer hover:bg-slate-600' onClick={()=>navigate("/")}>HOME</li>
        <li className='text-xs bg-black/80 px-4 py-2 rounded-xl cursor-pointer hover:bg-slate-600'onClick={()=>navigate("/collection")}>COLLECTIONS</li>
        <li className='text-xs bg-black/80 px-4 py-2 rounded-xl cursor-pointer hover:bg-slate-600'onClick={()=>navigate("/about")}>ABOUT</li>
        <li className='text-xs bg-black/80 px-4 py-2 rounded-xl cursor-pointer hover:bg-slate-600'onClick={()=>navigate("/contact")}>CONTACT</li>
      </div>
      ...
      {/* Search Bar */}
      {showSearch &&
        <div className='absolute top-full left-0 w-full h-[70px] bg-[#d8f6f988] flex items-center justify-center z-40'>
          <input
            type="text"
            placeholder='Search here'
            className='w-[60%] h-[50%] bg-[#233533] text-white placeholder:text-white px-6 py-2 rounded-full text-lg'
      ...
      {/* Mobile Bottom Nav */}
      <div className='md:hidden fixed bottom-0 left-0 w-full h-[50px] bg-[#191818] text-white flex justify-around items-center text-xs z-50'>
